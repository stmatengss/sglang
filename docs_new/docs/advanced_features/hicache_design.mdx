...
:::note Mooncake has two distinct modes
`--enable-hierarchical-cache --hicache-storage-backend mooncake` selects the
HiCache design documented on this page: SGLang owns GPU L1 and host L2, while
Mooncake provides L3.

`--radix-cache-backend mooncake` instead selects the direct Mooncake radix
backend. It bypasses SGLang's host pool and transfers page-aligned GPU KV
buffers directly to and from Mooncake, which owns DRAM/SSD placement. Direct
mode currently supports MHA/MLA with single-node TP1/TP2; it is incompatible
with HiCache, FlexKV, LMCache, hybrid SWA/SSM, speculative decoding, PP, CP,
PD disaggregation, and multi-node TP.
:::
