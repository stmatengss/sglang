...
Mooncake Transfer Engine supports multiple transports. SGLang applies **one protocol to the whole PD engine** (KV cache, auxiliary metadata, extra state, and staging). Select it with `--disaggregation-mooncake-protocol` or a `mooncake_<protocol>` backend alias.
...
`--disaggregation-mooncake-protocol` sets `MOONCAKE_PROTOCOL` (default `rdma`) before TransferEngine initialization. An explicit `--disaggregation-mooncake-protocol` wins over a `mooncake_<protocol>` alias. `mooncake_tcp` remains a supported alias and still forces TCP without RDMA HCA selection.
